...
public class ProductServiceTest {
    ...
    @Test
    public void shouldThrowExceptionItProductDoesNotExist2(){
    ...
    @Test
    public void shouldThrowExceptionItProductDoesNotExist3(){
        when(productRepository.findById(any()))
                .thenReturn(Optional.empty());
    ...
    @Test
    public void souldPassEmptyNameIdNull(){
        String empty = "";
        when(productRepository.findByNameContainingIgnoreCaseAndPriceGreaterThanEqualAndPriceLessThanEqual(
                eq(empty), any(Double.class), any(Double.class)))
                .thenReturn(singletonList(product));
    ...
    public void shouldFindPageOfProducts(){
        ...
        Page<Product> page = new PageImpl<Product>(singletonList(product), pageRequest, 1);
        when(productRepository.findByNameContainingIgnoreCaseAndPriceGreaterThanEqualAndPriceLessThanEqual(
                any(), any(), any(), eq(pageRequest)))
                .thenReturn(page);
        Page<Product> result = productService.search(null, 1d, 10d, pageRequest);
        assertThat(result, hasItem(product));
        verify(productRepository, times(1))
                .findByNameContainingIgnoreCaseAndPriceGreaterThanEqualAndPriceLessThanEqual(
                        any(), any(), any(), eq(pageRequest)
